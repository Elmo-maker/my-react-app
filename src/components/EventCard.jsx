...
export default function EventCard({ event }) {
  return (
    <div className="bg-gray-800 rounded-xl shadow-xl hover:shadow-indigo-500/20 overflow-hidden transition duration-300 transform hover:-translate-y-1 border border-gray-700/50">
      ...
      <img 
        src={event.img || "https://via.placeholder.com/400x200?text=No+Image"} 
      ...
      <div className="p-5">
        ...
        {/* JUDUL EVENT - pakai nama_event dari database */}
        <h3 className="text-2xl font-bold text-white mb-2 tracking-tight">
          {event.nama_event}
        </h3>
        ...
        <div className="space-y-2 text-sm text-gray-400 mb-4">
          ...
          {event.jumlah_tiket && (
            <p className="flex items-center space-x-2">
              ...
              <span>{event.jumlah_tiket} tiket tersedia</span>
            </p>
          )}
        </div>
        ...
        {/* HARGA */}
        <p className="text-lg text-indigo-400 font-bold border-t border-gray-700 pt-3">
          <span className="text-gray-400 font-normal">Mulai dari:</span> Rp {event.harga_tiket.toLocaleString('id-ID')}
        </p>
